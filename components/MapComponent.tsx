...
const stores = [
  ...
  },
  {
    ...
    address: "EQ/NORTE 102/103 LOTE A SALA 9B, ENTRADA A TÉRREO (MEZANINO)",
  ...
  },
  {
    ...
    address: "Av. Araucárias, Lotes 1835, 1905, 1955 e 2005 - Salas 333 e 334",
...
export default function MapComponent() {
  useEffect(() => {
    ...
    const init = async () => {
      // 1) injeta o CSS do CDN se não existir
      let link = document.querySelector(`link[href="${cssHref}"]`) as
        | HTMLLinkElement
        | null;
      if (!link) {
        ...
        // espera o load do CSS (com fallback timeout)
      ...
      try {
        ...
        // 3) init do mapa (garantir que exista #map)
        if (!document.getElementById("map")?.dataset.mapInitialized) {
          map = L.map("map").setView([-16.6996234, -49.2621436], 5);

          L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png")
            .addTo(map);

          // Forçar icon URLs para o CDN (opcional, mas evita problemas se você usar CSS local)
          ...
          stores.forEach((store) => {
            ...
            `;
            L.marker(store.position, { icon: blueIcon }).addTo(map).bindPopup(
              popupContent,
            );
          });
          ...
          unsubscribe = userLocation.subscribe((coords) => {
            if (coords && map) map.setView(coords, 11);
          });
        }
      } catch (err) {
    ...
    init(); // <-- NÃO esquecer de chamar a função async!

    return () => {
      if (unsubscribe) unsubscribe();
      if (map) map.remove();
      if (addedLink) {
        const l = document.querySelector(`link[href="${cssHref}"]`);
        if (l) l.remove();
      }
    };
  }, []);
